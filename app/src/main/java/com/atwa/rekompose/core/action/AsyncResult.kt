...
open class AsyncResult<T : Any>() : Action {

    lateinit var status: AsyncStatus<T>

    override fun isComplete() = runCatching { status !is Loading }.getOrElse { false }
    val data
        get() = (status as? Success)?.data ?: throw IllegalStateException("Data is accessed before it is set, make sure to check that status is success before accessing data field")
    val error
